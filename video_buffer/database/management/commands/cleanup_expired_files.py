    ...
    def handle(self, *args, **kwargs):
        ...
        for image in expired_images:
            if os.path.exists(image.image_file.path):
                self.stdout.write(f"Deleting image file: {image.image_file.path}")
        ...
        for video in expired_videos:
            if os.path.exists(video.video_file.path):
                self.stdout.write(f"Deleting video file: {video.video_file.path}")
                os.remove(video.video_file.path)  # Delete the file from storage
            video.delete()  # Remove the record from the database

        # Output the result
        self.stdout.write(self.style.SUCCESS(f"Deleted {image_count} expired images and {video_count} expired videos."))
